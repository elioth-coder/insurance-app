...
<x-layout :$breadcrumbs>
    <div class="flex space-x-3 min-h-screen">
        <div class="w-3/5 pb-6 pt-2">
            ...
            <x-card class="max-w-xl">
                <x-card-header>Verify Authentication</x-card-header>
                <form id="verify-authentication-form" method="POST">
                    @csrf
                    <x-forms.input-field class="w-full" name="serial" type="text" label="Enter/Scan COC serial number"
    ...
    <x-slot:script>
        <script>
            (function() {
                let form = document.getElementById('verify-authentication-form');
                form.addEventListener('submit', async (e) => {
                    ...
                    if(serial.status == 'Verified') {
                        window.location.href = `/search?query=${serial.serial}`;
                    ...
                    if (serial.status == 'For Verification') {
                        let result = await Swal.fire({
                            title: "Verify this authentication?",
                            icon: "warning",
                        ...
                        if (result.isConfirmed) {
                            ...
                            let response = await fetch("/api/authentication/verify_serial", {
                                method: 'POST',
                                body: formData,
                            });
                            let {
                    ...
                    if(serial.status=='Available') {
                        let result = await Swal.fire({
                            title: "Create new authentication?",
                            icon: "warning",
                            showCancelButton: true,
                            confirmButtonColor: "#3085d6",
                            cancelButtonColor: "#d33",
                        ...
                        if (result.isConfirmed) {
                            window.location.href = `/authentication/create?serial=${serial.serial}`;
                            Swal.fire({
                                title: 'Redirecting...',
                                allowOutsideClick: false,
                                showConfirmButton: false,
