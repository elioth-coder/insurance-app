        ...
        <x-card class="max-w-xl">
            ...
            <x-forms.form method="POST" action="/authentication">
                <div class="flex space-x-2">
                    <x-forms.select-field class="w-full" name="type" label="Registration Type"
                        placeholder="Select registration type" required>
                        <option value="new">New</option>
                        <option value="renewal">Renewal</option>
                    </x-forms.select-field>
                    <x-forms.select-field class="w-full" name="lto_mv_type" label="LTO MV Type"
                        placeholder="Select LTO MV Type" required>
                ...
                <hr class="my-1">
                <div class="flex space-x-2">
                    <x-forms.input-field class="w-full" name="plate_number" type="text" label="Plate no."
                        placeholder="Enter plate no." />
                    <x-forms.input-field class="w-full" name="mv_file_number" type="text" label="MV File no."
                        placeholder="Enter MV File no." />
                </div>
                <div class="flex space-x-2">
                    <x-forms.input-field class="w-full" name="serial_number" type="text" label="Serial/Chassis no."
                        placeholder="Enter serial/chassis no." required />
                    <x-forms.input-field class="w-full" name="engine_number" type="text" label="Motor/Engine no."
                ...
                <div class="flex space-x-2">
                    ...
                    <x-forms.input-field class="w-full" name="assured_name" type="text" label="Assured Name"
                        placeholder="Enter assured name" required />
                </div>
                <x-forms.textarea-field name="assured_address" label="Assured address"
                ...
                <hr class="my-1">
                <x-forms.input-field class="w-full" name="or_number" type="text" label="O.R. No."
                    placeholder="Enter O.R. no." required />
                <div class="flex space-x-2">
                    <x-forms.select-field class="w-full" name="coc_number" label="COC No." placeholder="Select COC no."
                        required>
                        @foreach ($seriesNumbers as $series)
                            <option value="{{ $series->series_number }}">{{ $series->series_number }}</option>
                        @endforeach
                    </x-forms.select-field>
                    <x-forms.input-field class="w-full" name="policy_number" type="text" label="Policy No."
                        placeholder="Enter Policy no." required />
                </div>
                <x-forms.select-field class="w-full" name="vehicle_type" label="Vehicle Type"
                    placeholder="Select vehicle type" required>
                    <option value="1">1 - Private Cars (including jeeps and AUVs) - 1 -Year(s) - 560.00</option>
                    <option value="2">2 - Light Medium Trucks (Own Goods) Not over 3,930 kgs. - 1 -Year(s) -
                        610.00</option>
                    <option value="3">3 - Heavy Trucks (Own Good), Private Buses over 3,930 kgs. - 1 -Year(s) -
                        1,200.00</option>
                    ...
                    <option value="9">9 - Light Medium Trucks (Own Goods) Not over 3,930 kgs. - 3 -Year(s) -
                        1,750.00</option>
                    <option value="10">10 - Heavy Trucks (Own Good), Private Buses over 3,930 kgs. - 3 -Year(s) -
                        3,440.00</option>
