                    ...
                    <tbody>
                        @forelse ($settings as $setting)
                            <tr class="group cursor-pointer">
                                <td class="group-hover:bg-violet-200 px-8 py-6">{{ $setting->key }}</td>
                                <td class="group-hover:bg-violet-200 px-8 py-6">{{ $setting->value }}</td>
                                <td class="group-hover:bg-violet-200 px-8 py-6">
                                    <x-forms.form class="hidden" method="POST" verb="DELETE"
                                        action="/settings/{{ $setting->id }}"
                                        id="delete-setting-{{ $setting->id }}-form">
                                        <button type="submit">
                                            Delete
                                        </button>
                                    </x-forms.form>

                                    <a href="/settings/{{ $setting->id }}/edit" title="Edit"
                                    ...
                                    <button onclick="confirmDelete({{ $setting->id }})" title="Delete"
                                        type="button"
        ...
        <script>
            const confirmDelete = async (id) => {
                ...
                if (result.isConfirmed) {
                    document.querySelector(`#delete-setting-${id}-form button`).click();
                }
            }
            (function() {
                setTimeout(() => {
                    if (document.getElementById("settings-table") && typeof DataTable !==
                        'undefined') {
                        const dataTable = new DataTable("#settings-table", {
                            fixedHeight: true,
                            searchable: true,
                            perPage: 50,
                        });
